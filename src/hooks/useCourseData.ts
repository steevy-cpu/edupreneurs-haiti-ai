...
async function fetchCourseData(subjectSlug: string): Promise<CourseData> {
...
export function useCourseData(subjectSlug: string) {
  return useQuery({
    queryKey: ['course-data', subjectSlug],
    queryFn: () => fetchCourseData(subjectSlug),
...
export function prefetchCourseData(queryClient: QueryClient, subjectSlug: string) {
