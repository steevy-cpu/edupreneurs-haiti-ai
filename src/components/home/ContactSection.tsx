...
// Lazy load ContactForm - only when section is visible
const ContactForm = lazy(() => import("@/components/home/ContactForm").then(m => ({ default: m.ContactForm })));

/**
 * Contact section with cards and lazy-loaded form.
 * Form only loads when section becomes visible.
 */
      ...
      <div className="container mx-auto">
        ...
        <div className="grid sm:grid-cols-2 gap-6 sm:gap-8 max-w-2xl mx-auto">
          ...
          <Card className="group hover:scale-[1.02] transition-all duration-300 ease-out hover:shadow-xl border-primary/20 hover:border-primary/40 bg-gradient-to-br from-card to-card/50 text-center">
            ...
            <CardContent>
              <a 
                href="mailto:contact@mon-edupreneur.com" 
        ...
        <div className="text-center mt-10">
          ...
          <div className="flex justify-center gap-4">
            ...
            <SocialIcon label="Instagram" disabled>
              <svg className="w-6 h-6 text-muted-foreground" fill="currentColor" viewBox="0 0 24 24">
            ...
            {/* TikTok — linked to @monedupreneur.com */}
            <SocialIcon label="TikTok" href="https://www.tiktok.com/@monedupreneur.com">
              <svg className="w-6 h-6 text-muted-foreground" fill="currentColor" viewBox="0 0 24 24">
...
/** Social icon — active when href provided, disabled otherwise */
function SocialIcon({ 
  children, 
  label, 
  href,
  ...
  return (
    <a 
      ...
      aria-label={isActive ? label : `${label} (bientôt disponible)`} 
      className={`w-12 h-12 rounded-full bg-muted/50 flex items-center justify-center hover:scale-[1.02] transition-all duration-300 ease-out shadow-md ${
        !isActive ? 'cursor-not-allowed opacity-50' : 'hover:bg-muted'
      }`}
    >
      {children}
    </a>
...
function ContactFormSkeleton() {
  return (
    <div className="max-w-xl mx-auto space-y-4 animate-pulse">
      ...
      <div className="h-12 bg-muted rounded-lg w-32 mx-auto" />
    </div>
  );
}
