...
export const BatchActivitiesRegenerator = ({ 
  ...
  const handleRegenerateAll = async () => {
    if (lessonsToRegenerate.length === 0) {
      toast.info("Aucune activité à régénérer!");
      return;
    }
    ...
    for (let i = 0; i < lessonsToRegenerate.length; i++) {
      ...
      try {
        ...
        // Call the activities generation edge function
        ...
        if (!data?.content) {
          throw new Error('Pas de contenu activités retourné');
        }

        // Update the lesson with new activities and clear flags
        ...
        console.error(`Error regenerating activities for ${lesson.title}:`, error);
        regenerationResults.push({
          lessonId: lesson.id,
          lessonTitle: lesson.title,
          success: false,
    ...
    if (errorCount === 0) {
      toast.success(`✓ ${successCount} activité${successCount > 1 ? 's' : ''} régénérée${successCount > 1 ? 's' : ''} avec succès!`);
    } else if (successCount === 0) {
      toast.error(`✗ Échec de la régénération de toutes les activités`);
    } else {
      toast.info(`${successCount} régénérée${successCount > 1 ? 's' : ''}, ${errorCount} erreur${errorCount > 1 ? 's' : ''}`);
    }
          ...
          <div className="flex items-center gap-3 text-xs flex-wrap">
            {successCount > 0 && (
              <span className="flex items-center gap-1 text-purple-600">
                ...
                {successCount} régénérée{successCount > 1 ? 's' : ''}
              </span>
            )}
            {errorCount > 0 && (
  ...
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        ...
        >
          ...
          <div className="flex flex-col items-start text-left">
            <span>Régénérer activités flaggées ({lessonsToRegenerate.length})</span>
            <span className="text-[10px] text-muted-foreground">
              {lessonsToRegenerate.length} activité{lessonsToRegenerate.length > 1 ? 's' : ''} nécessite{lessonsToRegenerate.length > 1 ? 'nt' : ''} régénération
            </span>
          </div>
        </Button>
      </AlertDialogTrigger>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Régénérer les activités flaggées?</AlertDialogTitle>
          <AlertDialogDescription asChild>
            <div className="space-y-4">
              <div className="p-3 rounded-lg bg-muted/50 space-y-2">
                <div className="flex items-center justify-between text-xs font-medium">
                  <span>🔄 Régénération batch pour {gradeLevel}</span>
                  <span>{lessonsToRegenerate.length} activités</span>
                </div>
                <p className="text-xs text-muted-foreground">
                  Ces activités ont été validées et identifiées comme ayant des éléments hors-contenu.
                </p>
              </div>

              <p className="text-sm">
                Cette action va régénérer {lessonsToRegenerate.length} activité{lessonsToRegenerate.length > 1 ? 's' : ''} 
                {' '}en utilisant le contenu de la leçon pour créer des activités alignées.
              </p>
              
              <div className="space-y-1">
                <p className="text-xs text-muted-foreground flex items-center gap-1">
                  <CheckCircle2 className="h-3 w-3 text-purple-600" />
                  Les activités sont sauvegardées automatiquement après chaque régénération.
                </p>
                <p className="text-xs text-muted-foreground flex items-center gap-1">
