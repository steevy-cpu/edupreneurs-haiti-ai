            ...
            <Tabs defaultValue="introduction" className="w-full">
              ...
              <TabsContent value="contenu" className="space-y-6">
                {lessonData.contenu !== "Contenu à venir..." && (
                  <div className="space-y-6">
                    {/* Check if content is already HTML or needs parsing */}
                    {lessonData.contenu.includes('<div') || lessonData.contenu.includes('<img') || lessonData.contenu.includes('<h2') || lessonData.contenu.includes('<h3') ? (
                      // Content is already HTML - render directly
                      <Card className="border border-border bg-card">
                        <CardContent className="pt-6 pb-6 px-6">
                          <div 
                            className="prose prose-lg max-w-none dark:prose-invert lesson-content"
                            dangerouslySetInnerHTML={{ __html: lessonData.contenu }}
                          />
                        </CardContent>
                      </Card>
                    ) : (
                      // Legacy content - parse and display in beautiful sections
